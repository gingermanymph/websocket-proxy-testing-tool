...
const Menu = ({ onSelect, active }) => {
    ...
    return (
        <div className='flex flex-col w-[30px] border-r border-light'>
            <div className='flex justify-center items-center h-[26px] border-b border-light '>
                <VscMenu className='h-[18px] w-[18px] text-light'/>
            </div>
            {menuItems.map((item, id) => (
                <button
                    className={`w-[30px] h-[30px] flex justify-center items-center text-light ${isSelected === item.type && 'selected' }`}
                    key={id}
                    onClick={() => {
                        onSelect(item.type);
